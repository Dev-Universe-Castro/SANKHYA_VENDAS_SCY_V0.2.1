...
/**
 * Busca todas as marcas do Sankhya para uma empresa específica
 */
async function buscarMarcasSankhya(idSistema: number, bearerToken: string): Promise<MarcaSankhya[]> {
...
/**
 * Sincroniza marcas de uma empresa específica
 */
export async function sincronizarMarcasPorEmpresa(idSistema: number, empresaNome: string): Promise<SyncResult> {
    ...
    try {
        console.log(`\n🚀🚀🚀 ================================================`);
        console.log(`🚀 SINCRONIZAÇÃO DE MARCAS`);
        ...
        const marcas = await buscarMarcasSankhya(idSistema, bearerToken);
        ...
        console.log(`📊 [Sync] Resumo: ${marcas.length} registros, ${inseridos} inseridos, ${atualizados} atualizados, ${registrosDeletados} deletados`);
        ...
        console.error(`❌ [Sync] Erro ao sincronizar marcas para ${empresaNome}:`, error);
