...
async function buscarRegioesSankhya(idSistema: number, bearerToken: string): Promise<RegiaoSankhya[]> {
    ...
    while (hasMoreData) {
        ...
        const PAYLOAD = {
            "requestBody": {
                "dataSet": {
                    ...
                    "entity": {
                        "fieldset": {
                            "list": "CODREG, NOMEREG, ATIVA, ANALITICA, CODTAB, CODTABMIN, FRETEMIN, PERCCUSVAR, PERCDESCFOB, CODVEND, PERCADICIONAL, PERCPREMIO, GERARRECEITA, VLRANTECIPACAO, GRAU, CODREGPAI, VENDAMIN"
                        }
                    }
                }
            }
        ...
        try {
            ...
            if (regioesPagina.length === 0 || !hasMoreResult) {
                hasMoreData = false;
                console.log(`🏁 [Sync] Última página atingida (hasMoreResult: ${hasMoreResult}, registros: ${regioesPagina.length})`);
            } else {
                currentPage++;
            ...
            if (error.response?.status === 401 || error.response?.status === 403) {
                ...
                console.log(`📊 [Sync] Progresso mantido: ${allRegioes.length} registros acumulados`);
    ...
    console.log(`✅ [Sync] Total de ${allRegioes.length} regiões recuperadas em ${currentPage} páginas`);
    return allRegioes;
}

/**
 * Executa o soft delete (marca como não atual) todas as regiões do sistema
 */
...
async function upsertRegioes(
    ...
    for (let i = 0; i < regioes.length; i += BATCH_SIZE) {
        ...
        for (const regiao of batch) {
         ...
         WHEN NOT MATCHED THEN
           INSERT (
             ...
             GERARRECEITA, VLRANTECIPACAO, GRAU, CODREGPAI, VENDAMIN,
           ...
           VALUES (
            ...
            if (result.rowsAffected && result.rowsAffected > 0) {
                const checkResult = await connection.execute(
                    `SELECT DT_CRIACAO FROM AS_REGIOES 
            WHERE ID_SISTEMA = :idSistema AND CODREG = :codReg`,
        ...
        console.log(`📦 [Sync] Processado lote ${Math.floor(i / BATCH_SIZE) + 1} de ${Math.ceil(regioes.length / BATCH_SIZE)}`);
    }
...
export async function sincronizarRegioesPorEmpresa(idSistema: number, empresaNome: string): Promise<SyncResult> {
    ...
    try {
        console.log(`\n🚀🚀🚀 ================================================`);
        console.log(`🚀 SINCRONIZAÇÃO DE REGIÕES`);
        ...
        console.log(`📊 [Sync] Resumo: ${regioes.length} registros, ${inseridos} inseridos, ${atualizados} atualizados, ${registrosDeletados} deletados`);
...
/**
 * Sincroniza regiões de todas as empresas ativas (uma por vez)
 */
export async function sincronizarTodasRegioes(): Promise<SyncResult[]> {
    console.log('🌐 [Sync] Iniciando sincronização de regiões de todas as empresas...');
    ...
    try {
        ...
        for (const empresa of empresas) {
            const resultado = await sincronizarRegioesPorEmpresa(
                empresa.ID_EMPRESA,
                empresa.EMPRESA
            );
...
export async function listarRegioes(idSistema?: number) {
    ...
    try {
        ...
        const result = await connection.execute(
            ...
            ${whereClause}
            ORDER BY E.ID_SISTEMA, E.CODREG`,
            params,
            { outFormat: oracledb.OUT_FORMAT_OBJECT }
        );
